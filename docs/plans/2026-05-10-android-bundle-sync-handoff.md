...
**Status as of 2026-05-10:** Implemented. `tools.sync_android_bundle` now maps verified Python bundles into BKMeeting's baseline `modelassets` pack or optional `qnnvalidationassets` pack and rewrites Android-facing manifest fields during sync.
...
| `vpcd` | `vpcd_balanced` | `build/model_bundle/vpcd/vpcd_balanced` | `modelassets/src/main/assets/models/punctuation/vpcd` |
| `vpcd` | `qnn_fixed_1024x128` | `build/model_bundle/vpcd/qnn_fixed_1024x128` | `qnnvalidationassets/src/main/assets/models/punctuation/vpcd/qnn_fixed_1024x128` |
...
- [x] Route VPCD fixed-shape QNN sync to `qnnvalidationassets` only when `--qnn-validation-assets` is explicit.
- [x] Add pytest coverage for Zipformer FP32 sync, VPCD fixed-shape sync gating, and VPCD fixed-shape validation-pack output.
...
Latest local run: `59 passed` for the full `test` suite.

## Next Decision Point

After BKMeeting strict QNN HTP validation and benchmark data exist, decide whether to promote `models/punctuation/vpcd/qnn_fixed_1024x128` into the production `models/punctuation/vpcd` namespace or keep the current alias split:
