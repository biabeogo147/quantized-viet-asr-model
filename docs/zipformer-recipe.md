...
Android handoff chứa hai representations canonical: FP32 fixed-shape cho CPU và `EPContext ONNX + model.bin` cho QNN HTP. Benchmark và app chính resolve cùng artifact IDs, manifests, support components và fixtures từ model repository; không có benchmark-only model export. Xem [Android operations](aihub-android-operations.md).
