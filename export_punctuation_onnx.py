...
def resolve_export_python(preferred_python: str | None) -> str:
    ...
    raise RuntimeError(
        "Khong tim thay Python co module 'transformers.onnx'. "
        "Hay cai dat mot phien ban transformers ho tro ONNX, "
        "hoac chi dinh --export-python toi interpreter phu hop."
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export assets/vietnamese-punc-cap-denorm-v1 to ONNX."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=5e-5,
        help="Validation tolerance used by transformers.onnx.",
    )
    parser.add_argument(
        "--clean",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-int8",
        action="store_true",
        help="Only export the FP32 ONNX file and skip INT8 quantization.",
    )
    parser.add_argument(
        "--export-python",
        help="Python executable used for the ONNX export step. If omitted, the script will auto-detect one.",
    )
    ...
    if os.path.abspath(export_python) != os.path.abspath(sys.executable):
        ...
        raise RuntimeError(
            "Interpreter hien tai khong co transformers.onnx va khong tim thay interpreter thay the."
        )
