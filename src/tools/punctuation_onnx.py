...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    generated_path = os.path.join(output_dir, "model.onnx")
    fp32_path = os.path.join(output_dir, DEFAULT_FP32_NAME)
    if os.path.exists(fp32_path):
        os.remove(fp32_path)
    os.replace(generated_path, fp32_path)
    print(f"Da xuat FP32 ONNX: {fp32_path}")

    if args.skip_int8:
        return

    int8_path = os.path.join(output_dir, DEFAULT_INT8_NAME)
    quantize_dynamic(
        model_input=fp32_path,
        model_output=int8_path,
        weight_type=QuantType.QInt8,
    )
    print(f"Da tao INT8 ONNX: {int8_path}")
