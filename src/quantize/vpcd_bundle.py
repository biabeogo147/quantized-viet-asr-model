...
from model_bundle.fixtures import read_jsonl, serialize_jsonl
...
def verify_bundle(
    *,
    ...
    tokenizer_to_model_ids = np.asarray(
        json.loads((bundle_dir / manifest.artifacts["tokenizer_to_model_id_map"]).read_text(encoding="utf-8")),
        dtype=np.int64,
    )
    model_to_tokenizer_ids = np.asarray(
        json.loads((bundle_dir / manifest.artifacts["model_to_tokenizer_id_map"]).read_text(encoding="utf-8")),
        dtype=np.int64,
    )
    ...
    for sample in iter_golden_samples(bundle_dir):
        ...
        if not np.array_equal(bridged_model_ids, expected_input_ids):
            raise AssertionError(
                f"Tokenizer encode mismatch for '{raw_text}': {bridged_model_ids.tolist()} != {expected_input_ids.tolist()}"
            )
        encode_verified += 1
